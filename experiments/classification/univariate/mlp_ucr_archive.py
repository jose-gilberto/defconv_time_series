...
for dataset in DATASETS:
    ...
    for experiment_number in range(NUMBER_OF_EXPERIMENTS):
        model = MLP(in_features=X_train.shape[-1], in_dim=1, num_classes=num_classes)
        
        checkpoint = ModelCheckpoint(
            monitor='train_loss',
            dirpath=f'{MODELS_DIR}',
            filename=f'mlp-{dataset}-{experiment_number}',
            save_top_k=1,
        ...
        results_data_dir['f1'].append(results[0]['f1_score'])

results_df = pd.DataFrame(results_data_dir)
results_df.to_csv(f'{RESULTS_DIR}mlp.csv', index=False)